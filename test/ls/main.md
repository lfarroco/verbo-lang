...
- `-a` includes hidden entries (names starting with a dot). With -a and -R combined, hidden directories are traversed recursively and printed with the same header format as any other directory. With -a and -R combined, hidden directories are traversed recursively just like any other directory.
